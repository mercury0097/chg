      ...
      if (cJSON_IsString(emotion)) {
        Schedule(
            [this, display, emotion_str = std::string(emotion->valuestring)]() {
              ...
              MaybeRunTouchEmotionMotion(emotion_str);
            });
      }
...
void Application::MainEventLoop() {
  while (true) {
    ...
    if (bits & MAIN_EVENT_CLOCK_TICK) {
      ...
      if (touch_emotion_followup_pending_ &&
          touch_emotion_followup_deadline_us_ > 0 &&
          esp_timer_get_time() > touch_emotion_followup_deadline_us_) {
        ESP_LOGI(TAG, "Touch: emotion follow-up timed out");
        ClearTouchEmotionFollowup();
      }
...
void Application::MaybeRunTouchEmotionMotion(const std::string &emotion) {
  if (!touch_emotion_followup_pending_ || emotion.empty()) {
    return;
  }

  if (touch_emotion_followup_deadline_us_ > 0 &&
      esp_timer_get_time() > touch_emotion_followup_deadline_us_) {
    ESP_LOGI(TAG, "Touch: skip follow-up motion, emotion arrived too late");
    ClearTouchEmotionFollowup();
    return;
  }
...
void Application::ClearTouchEmotionFollowup() {
  touch_emotion_followup_pending_ = false;
  touch_emotion_followup_deadline_us_ = 0;
}
