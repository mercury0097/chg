...
**配网方式**：
- 推荐使用 **BLE 配网**（小程序内完成）
- 机器人进入配网模式后会开启蓝牙，设备名沿用固件默认命名（`BOARD_NAME + MAC后缀`）
- 配网成功后通过 BLE 状态通知返回机器人 IP，小程序保存 `robot_ip`
- 保留 `http://192.168.4.1` 网页配网作为兜底方案
- **您只需要关注：控制接口（2.1-2.5）**
...
机器人进入配网模式后会启动 BLE 配网服务。小程序连接后写入 WiFi 信息，设备连接成功后通过 BLE 返回 IP。

**BLE 参数（当前固件）**：
- Service UUID：`0000FFF0-0000-1000-8000-00805F9B34FB`
- Write UUID：`0000FFF1-0000-1000-8000-00805F9B34FB`
- Notify/Read UUID：`0000FFF2-0000-1000-8000-00805F9B34FB`
- 设备名：沿用固件默认（`BOARD_NAME + MAC后缀`）
...
**状态返回**：
- `status=0`：正在连接，`message` 返回当前重试次数（`"1"` / `"2"` / `"3"`）
- `status=1`：连接成功，`message` 返回机器人 IP（如 `172.16.0.232`）
- `status=2`：连接失败，`message` 返回失败原因（如 `connect_failed`、`invalid_ssid`）

**网页配网（兜底）**：
- 当 BLE 不可用时，可引导用户使用 AP 网页配网：`http://192.168.4.1`
...
```
1. 小程序识别为首次连接，或用户主动添加新机器狗
2. 用户让机器人进入配网模式（设备侧开启 BLE 配网广播）
3. 小程序扫描并连接机器人蓝牙（设备名沿用固件默认）
4. 小程序写入 {"action":"setwifi","ssid":"...","password":"..."} 到 Write UUID
5. 机器人开始连接路由器；通过 Notify UUID 回传 status=0，message=当前重试次数（最多3次）
6. 连接成功后回传 status=1，message=机器人IP（例如 192.168.1.100）
7. 小程序保存 robot_ip 到本地存储，并切换到控制页
8. 后续控制通过 http://{robot_ip}:80/api/v1/device/* 调用接口
```
...
1. **配网阶段（推荐 BLE）**
   - 扫描目标设备名（沿用固件默认命名）
   - 连接后订阅 Notify UUID（`FFF2`）
   - 向 Write UUID（`FFF1`）写入 `{"action":"setwifi","ssid":"...","password":"..."}`
   - 收到 `status=1` 时保存 `robot_ip` 并跳转控制页
   - 收到 `status=2` 时展示失败原因并允许重试
...
**更新时间**：2026-01-20  
**更新内容**：
- 更新配网流程为“小程序 BLE 配网优先，AP 网页配网兜底”
- 增加 BLE 配网协议说明（`action/setwifi`、`status/message`）
- 完善小程序开发建议，补充 BLE 连接与状态处理要点
